...
### How it differs from Traditional Appsec Supply Chain Risks
...
- **T2 Tool Misuse.** Compromised tools, registries, or prompt templates pulled at runtime can cause agents to execute unintended actions while still appearing "authorized."
- **T11 Unexpected RCE and Code Attacks.** Poisoned plugins or packages introduced via supply chain channels can embed malicious payloads or hidden backdoors.
- **T12 Agent Communication Poisoning.** Malicious or altered MCP/A2A descriptors act as supply-chain vectors, allowing attackers to hijack communication flows and influence system behavior.
- **T13 Rogue Agents in Multi-Agent Systems.** A compromised agent inserted through registries or development-time coding assistants can persist as a rogue actor, enabling systemic damage or data exfiltration.

## Common Examples of Vulnerability

1. **Poisoned prompt templates autoloaded remotely.** An agent automatically pulls prompt templates from an external source that contain hidden malicious instructions (e.g., to exfiltrate data or perform destructive actions), leading it to execute harmful behavior without developer intent.
2. **Tool-descriptor injection.** An attacker embeds hidden instructions or malicious payloads into a tool's metadata or MCP/agent-card, which the host agent interprets as trusted guidance and acts upon.
3. **Impersonation and typosquatting.**   When an agent dynamically discovers or connects to external tools or services, it may be deceived by typosquatted endpoints (look-alike names chosen to trick resolution). Moreover, a malicious service can also impersonate a legitimate tool or agent (“symbol attack”), mimicking its identity, API, or behavior to gain trust and execute malicious actions.
4. **Post-install backdoor with local AI-CLI abuse.** A compromised package installs post-install hooks that run on developer machines, probing for secrets and invoking local AI assistants with crafted prompts to autonomously locate and exfiltrate credentials and files.
...
While these controls mitigate downstream ASI06 memory and context poisoning techniques, their purpose here is to enforce supply-chain integrity at ingestion time.
They prevent poisoned inputs from becoming part of the agent’s operational supply chain, which places this control squarely in ASI04.
...
- Perform red-team simulations against poisoned components (malicious plugin, poisoned prompt template, compromised collaborator agent) to validate defense efficacy.
...
## Scenario 4: Cross-Agent Privilege Escalation in GitHub Copilot (2025)

In this incident, a compromised Copilot agent became an upstream dependency inside a multi-agent development environment, injecting manipulated instructions and tool calls into collaborating agents and causing them to inherit elevated privileges and execute unauthorized actions across repositories and build pipelines. Because inter-agent trust lacked cryptographic identity, provenance validation, and admission controls, the poisoned agent effectively entered the **runtime supply chain**, contaminating downstream agents that relied on its outputs. This demonstrates how agent ecosystems create dynamic supply chains where a single compromised agent can propagate tainted logic, credentials, and actions at scale.

📎 [https://embracethered.com/blog/posts/2025/cross-agent-privilege-escalation-agents-that-free-each-other//](https://embracethered.com/blog/posts/2025/cross-agent-privilege-escalation-agents-that-free-each-other/)
...
3. [https://www.trustwave.com/en-us/resources/blogs/spiderlabs-blog/agent-in-the-middle-abusing-agent-cards-in-the-agent-2-agent-protocol-to-win-all-the-tasks/](https://www.trustwave.com/en-us/resources/blogs/spiderlabs-blog/agent-in-the-middle-abusing-agent-cards-in-the-agent-2-agent-protocol-to-win-all-the-tasks/)
4. [https://www.theregister.com/2025/07/21/vibe_coding_replit_ai_data_loss/](https://www.theregister.com/2025/07/21/vibe_coding_replit_ai_data_loss/)
