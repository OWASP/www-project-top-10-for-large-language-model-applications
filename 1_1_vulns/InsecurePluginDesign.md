...
Since plugins are, under normal circumstances, only accessed only by the LLM, exploitation is typically a result of another vulnerability such as excessive agency or direct or indirect prompt injection. However, plugins are still responsible for protecting themselves since side-channel attacks can still occur. 
...
1. A plugin accepts input parameters in a single text field instead of distinct input parameters that can be validated and sanitized.
2. A plugin accepts configuration strings instead of parameters that can override entire configuration settings.
3. A plugin accepts raw SQL or programming statements, which are more difficult to validate than parameters.
...
6. A plugin blindly trusts that the LLM output, which is the input to the plugin, correctly represents the expected to the initial prompt. 
...
4. [OpenAI Semantic Search Plugin Sample](https://github.com/openai/chatgpt-retrieval-plugin): A fully-featured solution for Semantic Search using embeddings and vector databases; demonstrates all authentication methods.
