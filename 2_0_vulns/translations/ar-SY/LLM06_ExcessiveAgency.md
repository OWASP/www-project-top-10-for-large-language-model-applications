...
#### 1. الإفراط في الوظائف
  امتلاك وكيل نموذج اللغة الكبير(LLM Agent) حق الوصول إلى إضافات تتضمن وظائف لا تُعد ضرورية لتشغيل عمليات النظام الأصلية. على سبيل المثال، يحتاج المطور إلى منح وكيل نموذج اللغة الكبير القدرة على قراءة المستندات من المستودع (Repository)، لكن الإضافة الخارجية التي اختار استخدامها تتضمن أيضًا إمكانية تعديل وحذف المستندات.
...
* التخلص من الإفراط في الوظائف عبر استخدام إضافة تنفذ فقط إمكانيات قراءة البريد الالكتروني،
* التخلص من الإفراط في الصلاحيات عبر التوثيق عبر خدمة البريد الالكتروني للمستخدم باستخدام جلسة OAuth بنطاق قراءة فقط، أو/و
* التخلص من الإفراط في الاستقلالية عبر الاشتراط على المستخدم أن يقوم بالمراجعة يدويًا والنقر على "إرسال" لكل رسالة يتم إعدادها بواسطة إضافة نموذج اللغة الكبير.

يمكن أيضا تقليل الضرر الناتج عبر تطبيق قيود على معدل إرسال الرسائل عبر واجهة الإرسال في البريد الإلكتروني نفسها.
