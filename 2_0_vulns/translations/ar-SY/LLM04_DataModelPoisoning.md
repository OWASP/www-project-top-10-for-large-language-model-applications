...
علاوة على ذلك، فإن النماذج التي يتم توزيعها عبر المستودعات البرمجية المشتركة أو منصات المصادر المفتوحة قد تنطوي على مخاطر تتجاوز تسميم البيانات؛ مثل احتوائها على برمجيات ضارة (Malware) تُزرع باستخدام تقنيات 'استغلال تسلسل البيانات الخبيث' (Malicious Pickling)، مما يؤدي إلى تنفيذ تعليمات برمجية ضارة فور تحميل النموذج. بالإضافة إلى ذلك، يمكن لتسميم البيانات أن يُمكّن من زرع باب خلفي (Backdoor) في النموذج لا يؤثر على سلوكه إلا عند تفعيل محفّز (Trigger) معين، مما يجعل اكتشافه واختباره أمراً بالغ الصعوبة، ويحوّل النموذج فعلياً إلى ما يشبه 'العميل النائم' (Sleeper Agent).
...
2. يمكن للمهاجمين حقن محتوى ضار بشكل مباشر ضمن عملية التدريب، مما يُضعف جودة مخرجات النموذج.
3. يُدخل المستخدمون، دون قصد، معلومات حساسة أو مملوكة أثناء التفاعل مع النموذج، مما قد يؤدي إلى كشفها في مخرجات لاحقة.
4. يزيد استخدام بيانات تدريب غير موثّقة من مخاطر إنتاج مخرجات متحيّزة أو خاطئة.
5. غياب القيود على الوصول إلى الموارد قد يسمح للنموذج بابتلاع بيانات غير آمنة، مما يؤدي إلى مخرجات متحيّزة.
