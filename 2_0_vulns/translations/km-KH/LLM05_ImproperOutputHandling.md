...
- កម្មវិធីផ្តល់ឱ្យ LLM នូវសិទ្ធិលើសពីអ្វីដែលបានបម្រុងទុកសម្រាប់អ្នកប្រើប្រាស់ ដែលអាចនាំឱ្យមានការដំឡើងសិទ្ធិ (escalation of privileges) ឬការបញ្ជាកូដពីចម្ងាយ (remote code execution)។
...
- មុខងារបន្ថែម (extensions) ពីភាគីទីបីមិនបានត្រួតពិនិត្យការបញ្ចូល (inputs) ឱ្យបានត្រឹមត្រូវ។
- កង្វះការធ្វើ output encoding ឱ្យបានត្រឹមត្រូវសម្រាប់បរិបទផ្សេងៗគ្នា (ឧទាហរណ៍ HTML, JavaScript, SQL)។
...
៤. លទ្ធផលរបស់ LLM ត្រូវបានប្រើដើម្បីបង្កើតជាទីតាំងឯកសារ (file paths) ដោយគ្មានការសម្អាត (sanitization) ត្រឹមត្រូវ ដែលអាចបណ្តាលឱ្យមានការវាយប្រហារតាមរយៈការឆ្លងកាត់ទីតាំងឯកសារ (path traversal vulnerabilities)។<br>
...
១. ចាត់ទុកម៉ូឌែល (model) ដូចជាអ្នកប្រើប្រាស់ដទៃទៀត ដោយអនុវត្តគោលការណ៍ zero-trust និងអនុវត្តការត្រួតពិនិត្យការបញ្ចូល (input validation) ត្រឹមត្រូវលើការឆ្លើយតបដែលមកពីម៉ូឌែល (model) ទៅកាន់មុខងារ backend។ <br>
២. អនុវត្តតាម OWASP ASVS (Application Security Verification Standard) guidelines ដើម្បីធានា input validation និង ការសម្អាត (sanitization) ប្រកបដោយប្រសិទ្ធភាព។<br>
...
៤. អនុវត្ត context-aware output encoding ដោយផ្អែកលើលទ្ធផលពី LLM នឹងត្រូវបានប្រើប្រាស់ (ឧទាហរណ៍៖ HTML encoding សម្រាប់ web content, SQL escaping សម្រាប់ database queries) ។<br>
...
៧. រៀបចំប្រព័ន្ធកត់ត្រា (robust logging) និង ប្រព័ន្ធត្រួតពិនិត្យ (monitoring systems) ឱ្យបានរឹងមាំ ដើម្បីស្វែងរកសញ្ញាមិនប្រក្រតីនៅក្នុងលទ្ធផលរបស់ LLM ដែលអាចបង្ហាញពីការប៉ុនប៉ងវាយប្រហារ។

#### ឧទាហរណ៍សេណារីយ៉ូនៃការវាយប្រហារ

##### សេណារីយ៉ូ #១

កម្មវិធីមួយប្រើប្រាស់មុខងារបន្ថែម (Extension) របស់ LLM ដើម្បីបង្កើតចម្លើយសម្រាប់មុខងារ chatbot ។ មុខងារបន្ថែមនេះក៏ផ្តល់នូវមុខងាររដ្ឋបាល (administrative functions) មួយចំនួនដែលអាចឱ្យ LLM មួយផ្សេងទៀតដែលមានសិទ្ធិខ្ពស់អាចចូលប្រើប្រាស់បាន។ គោលបំណងទូទៅរបស់ LLM គឺបញ្ជូនការឆ្លើយតបរបស់វាដោយផ្ទាល់ ដោយគ្មានការផ្ទៀងផ្ទាត់ទិន្នផលត្រឹមត្រូវទៅកាន់មុខងារបន្ថែម (Extension) ដែលបណ្តាលឱ្យមុខងារបន្ថែម (Extension) ត្រូវបានបិទដើម្បីជួសជុល។

##### សេណារីយ៉ូ #២

អ្នកប្រើប្រាស់ម្នាក់បានប្រើប្រាស់ឧបករណ៍សង្ខេបគេហទំព័រ (website summarizer tool) ដែលដំណើរការដោយ LLM ដើម្បីសង្ខេបអត្ថបទមួយ។ គេហទំព័រនោះមានបង្កប់នូវ prompt injection ដែលបញ្ជាឱ្យ LLM ចាប់យកខ្លឹមសាររសើប (sensitive content) ពីគេហទំព័រ ឬពីការសន្ទនារបស់អ្នកប្រើប្រាស់។ បន្ទាប់ពីទីនោះ LLM អាចបំប្លែងទិន្នន័យរសើបទាំងនោះជាកូដ (encode sensitive data) ហើយផ្ញើវាដោយគ្មានការត្រួតពិនិត្យភាពត្រឹមត្រូវនៃលទ្ធផល (output validation) ឬការចម្រោះទិន្នន័យ (filtering) ហើយបញ្ជូនវាទៅកាន់ Server របស់មេខ្លោងវាយប្រហារ (attacker-controlled server) ។
...
កម្មវិធី (Web app) មួយប្រើ LLM ដើម្បីបង្កើតមាតិកា (content) ចេញពីសារបញ្ជារបស់អ្នកប្រើប្រាស់ (user text prompts) ដោយគ្មានការសម្អាតលទ្ធផល ឬ ចម្លើយ។ អ្នកវាយប្រហារអាចផ្ញើសារបញ្ជាដែលបានរៀបចំទុកជាមុន ដែលបណ្តាលឱ្យ LLM បញ្ចេញមកវិញនូវ unsanitized JavaScript payload ដែលនាំឱ្យ XSS នៅពេលបង្ហាញ (rendered) នៅលើកម្មវិធីរុករក (Browser) របស់អ្នករងគ្រោះ។ ការផ្ទៀងផ្ទាត់សារបញ្ជា (prompts)មិនគ្រប់គ្រាន់បានបើកឱ្យការវាយប្រហារនេះ។

##### សេណារីយ៉ូ #៥

LLM ត្រូវបានប្រើដើម្បីបង្កើត dynamic email templates សម្រាប់យុទ្ធនាការទីផ្សារ។ អ្នកវាយប្រហារបានបញ្ជាឱ្យ LLM បញ្ចូលកូដ JavaScript ដែលមានបំណងអាក្រក់ទៅនៅក្នុងខ្លឹមសារអ៊ីមែល។ ប្រសិនបើកម្មវិធីមិនមានការសម្អាតលទ្ធផលរបស់ LLM បានត្រឹមត្រូវទេ នេះអាចនាំឱ្យមានការវាយប្រហារបែប XSS ទៅលើអ្នកទទួលបើកមើលអ៊ីមែលនៅក្នុងកម្មវិធីអ៊ីមែល (Email clients) ដែលមានចន្លោះប្រហោង។
