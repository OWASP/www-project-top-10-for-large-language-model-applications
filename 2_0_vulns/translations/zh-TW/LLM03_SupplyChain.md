...
本文所討論的一部分風險也在「LLM04 資料與模型毒化」中提及。本項目著重於風險的供應鏈層面。
...
8. 在供應的模型與資料上採用異常檢測 (Anomaly Detection) 與對抗魯棒性測試，可偵測竄改與毒化攻擊。此作法於 "LLM04 資料與模型毒化" 有提及，理想狀況是將其納入 MLOps 與 LLM 流程。但考量技術尚新穎，或可先於對抗性測試中實施。
