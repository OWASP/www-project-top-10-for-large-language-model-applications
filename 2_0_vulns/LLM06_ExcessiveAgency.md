...
#### 7. Excessive Autonomy
  ...
  An autonomous AI agent executes tool invocations generated by another agent without independently validating the requested action or authorization context. Trusting inter-agent instructions can result in privilege escalation or unauthorized operations across multi-agent workflows.

#### 8. Excessive Permissions

An MCP server or external tool is granted broad permissions that exceed the intended operational scope. A compromised or manipulated agent can leverage these permissions to access or modify resources beyond its authorized responsibility.

#### 9. Excessive Functionality

A planning or orchestration agent is permitted to generate arbitrary execution plans that downstream agents execute without independent validation. Manipulated workflow plans may trigger unintended actions across autonomous systems.
...
#### 9. Validate inter-agent communication

Treat communications between autonomous agents as untrusted input. Validate messages, execution plans, and delegated tasks before allowing downstream agents to perform actions.

#### 10. Enforce runtime authorization

Authorize every tool invocation at execution time regardless of which agent generated the request. Runtime authorization should be enforced independently of LLM reasoning or workflow state.

#### 11. Validate execution plans

Verify AI-generated task plans before execution. Applications should ensure generated workflows comply with organizational security policies and authorized operational boundaries.

#### 12. Apply policy enforcement to autonomous actions

Use deterministic policy enforcement to evaluate high-impact actions before execution. Runtime policy engines should independently validate tool selection, parameters, permissions, and execution context.

#### 13. Isolate agent responsibilities

Separate planning, reasoning, retrieval, and execution responsibilities into distinct security boundaries. Individual agents should only receive the minimum permissions necessary to perform their assigned function.
...
An autonomous planning agent generates a workflow instructing an execution agent to provision cloud resources. Because the execution agent trusts the generated plan without validating authorization or policy constraints, unauthorized infrastructure is created within the production environment.

#### Scenario #3

A compromised retrieval agent embeds malicious instructions within information returned from an external tool. Downstream agents interpret the response as trusted context and execute unauthorized actions across the autonomous workflow.

#### Scenario #4

An application exposes an MCP server with administrative capabilities intended for operational maintenance. A manipulated AI agent invokes privileged functions that exceed its intended scope because runtime authorization is not independently enforced.

#### Scenario #5

An AI agent generates a multi-step execution plan that includes privileged operations across several downstream agents. Because each agent assumes the workflow originated from a trusted planner, unauthorized actions propagate throughout the autonomous system without human approval.
...
7. [Model Context Protocol (MCP) Specification](https://modelcontextprotocol.io/specification): **Model Context Protocol**
8. [OWASP Application Security Verification Standard (ASVS)](https://owasp.org/www-project-application-security-verification-standard/): **OWASP**
9. [Artificial Intelligence Risk Management Framework (AI RMF 1.0)](https://www.nist.gov/itl/ai-risk-management-framework): **National Institute of Standards and Technology (NIST)**
10. [Anthropic – Building Effective AI Agents](https://www.anthropic.com/engineering/building-effective-agents): **Anthropic**
11. [Sandboxing Agentic AI Workflows with WebAssembly](https://developer.nvidia.com/blog/sandboxing-agentic-ai-workflows-with-webassembly/): **NVIDIA**
