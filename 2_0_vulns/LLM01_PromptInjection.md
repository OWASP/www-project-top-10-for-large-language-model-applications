...
#### 7. Conduct adversarial testing and attack simulations
  ...
  Applications using autonomous agents should implement additional controls beyond traditional prompt filtering.

#### 8. Additional Mitigations for Agentic AI

  Treat all inter-agent communications as untrusted input.
  Validate MCP tool responses before forwarding them to downstream agents.
  Isolate persistent agent memory by security boundary.
  Enforce authorization before every tool invocation regardless of message origin.
  Validate execution plans before agent handoff.
  Prevent recursive propagation of injected instructions across multi-agent workflows.
  Apply runtime policy enforcement to all autonomous actions.
...
#### Scenario #10: Agent-to-Agent Prompt Injection

An attacker compromises one AI agent or workflow component and injects malicious instructions into messages sent to downstream agents. Because the receiving agent trusts inter-agent communication, the malicious instructions propagate across the workflow, resulting in unauthorized tool execution, privilege escalation, or disclosure of sensitive information.

#### Scenario #11: MCP Tool Output Injection

An attacker compromises an external tool or Model Context Protocol (MCP) server and embeds malicious instructions within otherwise legitimate tool output. When the LLM consumes the response without validation, it interprets the injected content as trusted context and performs unintended actions.

#### Scenario #12: Memory Injection

An attacker poisons an AI agent's persistent memory during an earlier interaction. When the memory is later retrieved, the agent unknowingly executes attacker-controlled instructions despite the current user prompt appearing benign.

#### Scenario #13: Multi-Agent Prompt Propagation

An attacker injects malicious content into one stage of a multi-agent workflow. The compromised agent forwards manipulated plans, summaries, or reasoning artifacts to downstream agents, allowing the prompt injection to propagate across the autonomous workflow.

#### Scenario #14: Planning Artifact Injection

An attacker modifies intermediate planning artifacts such as task lists, execution plans, or workflow state generated by autonomous agents. Downstream agents consume the poisoned artifacts and execute attacker-controlled objectives while assuming the plans originated from trusted system reasoning.
