...
6. LLM-generated tool invocations are executed without validating the requested tool, parameters, or authorization context, resulting in unauthorized actions or privilege escalation.
7. Structured outputs (such as JSON or XML) generated by an LLM are assumed to be trustworthy and consumed directly by downstream applications without schema validation, resulting in unintended system behavior.
8. Outputs generated by autonomous AI agents are executed without enforcing runtime security policies, allowing malformed or adversarial responses to trigger unintended workflows.
...
8. Validate all model-generated outputs against deterministic runtime policies before executing downstream actions.
9. Verify tool invocations by validating tool selection, parameters, authorization context, and execution policies before execution.
10. Validate structured outputs using predefined schemas and reject responses that violate expected formats or policy constraints.
11. Apply runtime authorization checks independently of the model before invoking privileged operations or external systems.
...
#### Scenario #7
  ...
  An autonomous AI agent generates a tool invocation to modify cloud infrastructure. The application executes the generated tool request without validating the requested operation or authorization context, resulting in unauthorized resource modification.

#### Scenario #8

  An application consumes structured JSON output generated by an LLM and forwards it directly to downstream services. Because the output is not validated against the expected schema, malformed fields cause unintended system behavior and execution of unauthorized actions.

#### Scenario #9

  A multi-agent application allows one AI agent to generate execution plans for another. A compromised planning agent produces malicious workflow instructions that are executed without runtime validation, resulting in unauthorized tool execution across the autonomous workflow.
...
11. [OWASP Application Security Verification Standard (ASVS) – Validation, Sanitization and Encoding](https://owasp.org/www-project-application-security-verification-standard/): **OWASP**
